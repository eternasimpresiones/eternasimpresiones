...
@Component
public class GuiaRemisionBuilder extends ProcesadorDocumento{
	...
	public void crearDetalles() {
		...
		} else if (getDocumento().getDetalle_Guia() != null && getDocumento().getDetalle_Guia().size() > 0) {
			for (_CpeGuiaRemisionDetalleBean item : getDocumento().getDetalle_Guia()) {
				item.setId_documento(getDocumento().getId_documento());
				item.setId_empresa(getDocumento().getId_empresa());
				item.setITEM(count_item);
				count_item++;
	...
	public String getRutaDocEmpresaEmisora() {
		...
		if (getDocumento().getCod_tipo_documento().equals(Constantes.tipoDocGuiaRemisionRemitente)) {
			dirEmpresaEmisora = dirEmpresaEmisora + File.separator + "GUIA REMISION";
			tipo_Operacion_Sunat.setCodigo(Constantes.codigoDeclaracionGuiaRemisionRemitente);
		} else {
			dirEmpresaEmisora = dirEmpresaEmisora + File.separator + "GUIA TRANSPORTISTA";
	...
	public Integer procesarDocumento() throws Exception {
		...
		crearDetalles();
		Integer resp = detalle_DocumentoService.insertarDocumento_Detalle_GuiaPorLista(getDocumento().getDetalle_Guia());
		enviarDocSunat();
		return resp;
	}
