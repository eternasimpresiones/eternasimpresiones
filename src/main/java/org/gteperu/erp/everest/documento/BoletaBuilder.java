...
@Component
public class BoletaBuilder extends ProcesadorDocumento {
	...
	protected void crearDocumento(String estadoDocumento) {
		...
		this.getDocumento().setRuc_empresaEmisora(company.getNro_documento_empresa());
		this.getDocumento().setUserSol_empresaEmisora(company.getUsuario_sol_empresa());
		this.getDocumento().setPassSol_empresaEmisora(company.getPass_sol_empresa());
		this.getDocumento().setPassFirma_empresaEmisora(company.getPass_firma_empresa());
		this.getDocumento().setNro_documento_empresa(company.getNro_documento_empresa());
		this.getDocumento().setTipo_documento_empresa(company.getTipo_doc_empresa());
		this.getDocumento().setNombre_comercial_empresa(company.getNombre_comercial_empresa());
		this.getDocumento().setCodigo_ubigeo_empresa(company.getUbigeo().getCodigo());
		this.getDocumento().setDireccion_empresa(company.getDireccion_empresa());
		this.getDocumento().setDepartamento_empresa(company.getUbigeo().getDepartamento());
		this.getDocumento().setProvincia_empresa(company.getUbigeo().getProvincia());
		this.getDocumento().setDistrito_empresa(company.getUbigeo().getDistrito());
		this.getDocumento().setRazon_social_empresa(company.getRazon_social_empresa());
		this.getDocumento().setIfacturacionPse(company.getIfacturacionpse());

		this.getDocumento().setNro_documento_cliente(cliente.getNro_doc());
		this.getDocumento().setTipo_documento_cliente(cliente.getTipo_doc());
		this.getDocumento().setRazon_social_cliente(cliente.getRazon_social());
		this.getDocumento().setCod_ubigeo_cliente(cliente.getUbigeo().getCodigo());
		this.getDocumento().setDepartamento_cliente(cliente.getUbigeo().getDepartamento());
		this.getDocumento().setProvincia_cliente(cliente.getUbigeo().getProvincia());
		this.getDocumento().setDistrito_cliente(cliente.getUbigeo().getDistrito());
		this.getDocumento().setCiudad_cliente(cliente.getUbigeo().getProvincia());
		//this.getDocumento().setFecha_documento(new Timestamp(System.currentTimeMillis()));
		
		getDocumento().setAmbiente_operacion(company.getAmbiente_operacion());

		Numero_Letras num_to_letras = new Numero_Letras();
		String total_letritas = num_to_letras.Convertir(Double.toString(this.getDocumento().getTotal()), true,getDocumento().getCod_moneda());
		this.getDocumento().setTotal_letras(total_letritas);
		this.getDocumento().setDirDocumentoEmpresaEmisora(this.getRutaDocEmpresaEmisora());
		this.getDocumento().setEstado(Integer.parseInt(estadoDocumento));
	}

	@Override
	protected void crearDetalles() {
		Integer count_item = 1;
		if (this.getDocumento().getDetalle() != null && this.getDocumento().getDetalle().size() > 0) {
			for (_DocumentoCpe_DetalleBean item : this.getDocumento().getDetalle()) {
				item.setId_documento(this.getDocumento().getId_documento());
				item.setId_empresa(this.getDocumento().getId_empresa());
	...
	protected void enviarDocSunat() {
		...
		hilo.setDocumento(this.getDocumento());
	...
	protected void initialize() throws Exception {
		...
		company.setId_empresa(this.getDocumento().getId_empresa());
		...
		cliente.setId_cliente(this.getDocumento().getId_cliente_cpe());
