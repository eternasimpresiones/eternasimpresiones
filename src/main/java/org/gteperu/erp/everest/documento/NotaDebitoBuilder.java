...
@Component
public class NotaDebitoBuilder extends ProcesadorDocumento {
	...
	protected void crearDocumento(String estadoDocumento)  throws Exception {
		if (getDocumento().getTipo_comprobante_modifica() == null
				...
				|| getDocumento().getCod_tipo_motivo() == null) {
			throw new Exception("La nota de debito tiene campos que estan incompletos");
		}
	...
	//	getDocumento().setFecha_documento(new Timestamp(System.currentTimeMillis()));
		...
		getDocumento().setId_cliente_cpe(cliente.getId_cliente());
		...
		if (getDocumento().getCod_tipo_motivo().equals("02")) {
			Double total = 0.0;
			Double igv = 0.0;
			for (_DocumentoCpe_DetalleBean det : getDocumento().getDetalle()) {
			...
			getDocumento().setTotal(total);
			getDocumento().setTotal_igv(igv);
		}
	}
	...
	protected void initialize() throws Exception {
		...
		cliente.setId_cliente(getDocumento().getClientes().getId_cliente());
